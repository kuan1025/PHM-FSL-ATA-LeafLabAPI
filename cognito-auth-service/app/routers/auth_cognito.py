...
COOKIE_DOMAIN = settings.COOKIE_DOMAIN or "localhost"
...
def signup(body: SignUpBody):
    ...
    secret_hash = _secret_hash_for(body.username)
    if secret_hash:
    ...
    try:
        ...
        raise HTTPException(400, f"Invalid password policy: {exc}")
...
def login(form: OAuth2PasswordRequestForm = Depends()):
    ...
    secret_hash = _secret_hash_for(form.username)
    if secret_hash:
...
def oauth_callback(request: Request, code: str, state: Optional[str] = None):
    ...
    out = JSONResponse({"message": "Login with Google via Cognito success", "tokens": tokens})
    out.delete_cookie("oauth_state", domain=COOKIE_DOMAIN, path="/")
    return out
